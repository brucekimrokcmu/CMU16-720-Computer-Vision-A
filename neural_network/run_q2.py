    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                hl1 = forward(xb,params,name='layer1',activation=sigmoid)
                probs = forward(hl1, params, name='output', activation=softmax)
                loss1, acc = compute_loss_and_acc(yb, probs)
                v[i,j] = orgn_wght - eps
                hl1 = forward(xb,params,name='layer1',activation=sigmoid)
                probs = forward(hl1, params, name='output', activation=softmax)
                loss2, acc = compute_loss_and_acc(yb, probs)
                v[i,j] = orgn_wght
        ...
        for i in range(v.shape[0]):
            ...
            hl1 = forward(xb,params,name='layer1',activation=sigmoid)
            probs = forward(hl1, params, name='output', activation=softmax)
            loss3, acc = compute_loss_and_acc(yb, probs)
            v[i] = orgn_wght - eps
            hl1 = forward(xb,params,name='layer1',activation=sigmoid)
            probs = forward(hl1, params, name='output', activation=softmax)
            loss4, acc = compute_loss_and_acc(yb, probs)
            v[i] = orgn_wght
            params['grad_'+k][i] = (loss3 - loss4)/(2*eps)
